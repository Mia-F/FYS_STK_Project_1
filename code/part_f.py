...
if __name__ == '__main__':
    ...
    # Compute Lasso regression errors
    for i in range(max_degree):
        error_train_lasso[i], error_test_lasso[i] = crossval_lasso(x, y, z_noise, i+1, k, alpha)

    # Plotting
    fig5, ax5 = plt.subplots()
    # Previous error plots (bootstrap and CV for OLS)
    ax5.plot(degrees, error_train_noise, label='Bootstrap Train', color=colors[0])
    ax5.plot(degrees, error_test_noise, label='Bootstrap Test', color=colors[1])
    ax5.plot(degrees, error_train_noise_cv, label='CV OLS Train', color=colors[2])
    ax5.plot(degrees, error_test_noise_cv, label='CV OLS Test', color=colors[3])
    # Lasso regression error plots
    ax5.plot(degrees, error_train_lasso, label='CV Lasso Train', color=colors[4], linestyle='-.')
    ax5.plot(degrees, error_test_lasso, label='CV Lasso Test', color=colors[5], linestyle='-.')
    ...
    fig5.savefig("../LaTeX/Images/lasso_cv.png")

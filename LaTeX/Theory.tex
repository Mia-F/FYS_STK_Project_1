...
\noindent The $R^2$-score function is another popular error metric, it computes the coefficient of determination, which is an indicator of
how accurately the model can predict future samples. The score ranges from -1 to 1, where a score of 1 indicates a perfect fit, and a
score of -1 indicates a perfectly inadequate model. A constant model that always predicts the expected value of the target variable,
regardless of the input features, would receive an $R^2$-score of 0 (Kilde 3AM).

\noindent The r2 score is defined as 
